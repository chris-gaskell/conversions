...
#' @export
predict_retest <- function(method = "crawford", t1.score,
  ...
  if (missing(method)) {stop("'method' argument for calculating predict_retest required.")}
 ...
 class(result) <- 'predict_retest'
 return(result)
}

#' @export
print.predict_retest <- function(x, ...) {
...
predict_retest(method = "crawford", 100, 103, 10, 109, 8, 0.76, 100, .95)
